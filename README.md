...
| `dimmer` | boolean | no | Enable brightness control (levels 1–10, with 0 as off). When a light is triggered by a gateway scenario or automation rule, the gateway sends an extended packet (`*1*1000#X*WHERE##`) that is handled transparently. |
